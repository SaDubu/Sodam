# Sodam — 영상 전사·요약·소개글 생성 파이프라인

Sodam은 영상 파일을 외부 클라우드로 전송하지 않고, 사용자의 PC에서 **FFmpeg → faster-whisper → Ollama/Qwen**을 연결해 전사와 콘텐츠 결과물을 만드는 애플리케이션입니다. 기존의 사실 중심 `summary`를 유지하면서, 같은 전사문에서 시청자의 호기심을 유도하는 `introduction`을 선택적으로 생성할 수 있습니다.
